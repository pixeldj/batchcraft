...
- steps;
- guidance;
- output-prefix target mapping, with the prefix generated by batchcraft rather than edited in the Batch.
...
Output naming is not editable Batch intent. Run creation derives and freezes each Job's output prefix
from Run and Job identity.
...
`Load Run as Batch` is implemented. It restores editable intent from a historical Run, requires a fresh
Preview, and creates a new Run through the normal compilation path. Random intent generates fresh
per-Job seeds on that Preview rather than reusing the historical assignments.

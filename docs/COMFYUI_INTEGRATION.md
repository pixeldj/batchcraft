...
The Workflow Profile becomes the stable mapping used by Jobs.

## Workflow Snapshotting

A Run must preserve the workflow snapshot actually used for execution, along with a content hash.

If a user later edits or replaces the Workflow Profile, historical Runs remain interpretable and reproducible.

## Input Images

Reference images are owned by batchcraft on the Mac.
...
The Job manifest retains the batchcraft asset identity and hash rather than treating the temporary ComfyUI filename as authoritative provenance.

## Output Images

ComfyUI may create its normal native output files on the Windows workstation.

After a Job succeeds, batchcraft retrieves the relevant outputs and stores application-owned copies under the Run directory on the Mac.
